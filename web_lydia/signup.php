<?php
session_start();
$bdd = new PDO('mysql:host=localhost;dbname=datamoodbd;charset=utf8mb4;port=3307', 'root', 'root');
$bdd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);

// Rediriger si déjà connecté
if (isset($_SESSION['user_id'])) {
    header('Location: dashboard-pro.php');
    exit;
}

$error_message = '';
$success_message = '';

// Traiter la soumission du formulaire
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $nom = trim($_POST['nom'] ?? '');
    $prenom = trim($_POST['prenom'] ?? '');
    $email = trim($_POST['email'] ?? '');
    $motdepasse = $_POST['motdepasse'] ?? '';
    $motdepasse2 = $_POST['motdepasse2'] ?? '';

    // Validations
    if (empty($nom) || empty($prenom) || empty($email) || empty($motdepasse) || empty($motdepasse2)) {
        $error_message = 'Tous les champs sont requis';
    } elseif (strlen($nom) < 2) {
        $error_message = 'Le nom doit avoir au moins 2 caractères';
    } elseif (strlen($prenom) < 2) {
        $error_message = 'Le prénom doit avoir au moins 2 caractères';
    } elseif (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        $error_message = 'Email invalide';
    } elseif (strlen($motdepasse) < 8) {
        $error_message = 'Le mot de passe doit avoir au moins 8 caractères';
    } elseif (!preg_match('/[0-9]/', $motdepasse)) {
        $error_message = 'Le mot de passe doit contenir au moins 1 chiffre';
    } elseif (!preg_match('/[!@#$%^&*]/', $motdepasse)) {
        $error_message = 'Le mot de passe doit contenir au moins 1 caractère spécial (!@#$%^&*)';
    } elseif ($motdepasse !== $motdepasse2) {
        $error_message = 'Les mots de passe ne correspondent pas';
    } else {
        try {
            $stmt = $bdd->prepare('SELECT id FROM users WHERE email = ?');
            $stmt->execute([$email]);

            if ($stmt->rowCount() > 0) {
                $error_message = 'Cet email est déjà utilisé';
            } else {
                // Hacher le mot de passe
                $hashedPassword = password_hash($motdepasse, PASSWORD_BCRYPT);

                // Insertion dans 'users'
                $stmt = $bdd->prepare('
                    INSERT INTO users (nom, prenom, email, motdepasse) 
                    VALUES (?, ?, ?, ?)
                ');

                $stmt->execute([$nom, $prenom, $email, $hashedPassword]);
                $success_message = 'Compte créé avec succès ! Redirection vers la connexion...';
                
                // Redirection après 2 secondes
                header('Refresh: 2; url=login.php');
            }
        } catch (PDOException $e) {
            $error_message = 'Erreur lors de la création du compte';
            error_log('Erreur signup: ' . $e->getMessage());
        }
    }
}
?>
<!DOCTYPE html>
<html lang="fr">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>S'inscrire - Data Moode</title>
    <style>
        * {
            margin: 0;
            padding: 0;
            box-sizing: border-box;
        }

        body {
            font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen, Ubuntu, Cantarell, sans-serif;
            background: linear-gradient(135deg, #6B3FB8 0%, #9333ea 100%);
            min-height: 100vh;
            display: flex;
            align-items: center;
            justify-content: center;
            padding: 20px;
        }

        .container {
            width: 100%;
            max-width: 500px;
        }

        .form-box {
            background: white;
            border-radius: 16px;
            padding: 50px;
            box-shadow: 0 20px 60px rgba(0,0,0,0.3);
        }

        .form-header {
            text-align: center;
            margin-bottom: 40px;
        }

        .logo {
            display: flex;
            align-items: center;
            justify-content: center;
            gap: 10px;
            margin-bottom: 20px;
        }

        .logo-icon {
            width: 50px;
            height: 50px;
            border-radius: 8px;
            background: #ffffffff;
            display: flex;
            align-items: center;
            justify-content: center;
            font-size: 28px;
            color: white;
            overflow: hidden;
        }

        .logo-icon img {
            width: 100%;
            height: 100%;
            object-fit: cover;
        }

        .logo-text {
            font-size: 24px;
            font-weight: bold;
            color: #6B3FB8;
        }

        .form-header h1 {
            font-size: 28px;
            margin-bottom: 10px;
            color: #333;
        }

        .form-header p {
            color: #666;
            font-size: 14px;
        }

        .form-group {
            margin-bottom: 20px;
        }

        label {
            display: block;
            margin-bottom: 8px;
            font-weight: 600;
            color: #333;
            font-size: 14px;
        }

        input {
            width: 100%;
            padding: 12px;
            border: 2px solid #e5e7eb;
            border-radius: 8px;
            font-size: 14px;
            font-family: inherit;
            transition: all 0.3s ease;
        }

        input:focus {
            outline: none;
            border-color: #6B3FB8;
            box-shadow: 0 0 0 3px rgba(107, 63, 184, 0.1);
        }

        input.valid {
            border-color: #4CAF50;
            background-color: #f0fff0;
        }

        input.invalid {
            border-color: #f44336;
            background-color: #fff0f0;
        }

        .message-alert {
            padding: 12px;
            border-radius: 8px;
            margin-bottom: 20px;
            display: none;
            font-size: 14px;
        }

        .alert-success {
            background: #dcfce7;
            color: #166534;
            border: 1px solid #86efac;
            display: block;
        }

        .alert-error {
            background: #fee2e2;
            color: #991b1b;
            border: 1px solid #fca5a5;
            display: block;
        }

        button {
            width: 100%;
            padding: 12px;
            background: #6B3FB8;
            color: white;
            border: none;
            border-radius: 8px;
            font-size: 16px;
            font-weight: 600;
            cursor: pointer;
            transition: all 0.3s ease;
        }

        button:hover {
            opacity: 0.9;
            transform: translateY(-2px);
        }

        button:disabled {
            opacity: 0.5;
            cursor: not-allowed;
        }

        .form-footer {
            text-align: center;
            margin-top: 20px;
        }

        .form-footer p {
            font-size: 14px;
            color: #666;
        }

        .form-footer a {
            color: #6B3FB8;
            text-decoration: none;
            font-weight: 600;
        }

        .form-footer a:hover {
            text-decoration: underline;
        }

        .back-link {
            display: inline-block;
            margin-bottom: 20px;
            text-decoration: none;
            color: white;
            font-size: 14px;
            font-weight: 600;
        }

        .back-link:hover {
            opacity: 0.8;
        }
    </style>
</head>
<body>
    <div class="container">
        <a href="index.php" class="back-link">← Retour à l'accueil</a>

        <div class="form-box">
            <div class="form-header">
                <div class="logo">
                    <div class="logo-icon">
                        <img src="images/logoDataMood.png" alt="Data Moode Logo">
                    </div>
                    <div class="logo-text">Data Moode</div>
                </div>
                <h1>Créer un compte</h1>
                <p>Rejoignez Data Moode gratuitement</p>
            </div>

            <?php if (!empty($error_message)): ?>
                <div class="message-alert alert-error">
                    <?php echo htmlspecialchars($error_message); ?>
                </div>
            <?php endif; ?>

            <?php if (!empty($success_message)): ?>
                <div class="message-alert alert-success">
                    <?php echo htmlspecialchars($success_message); ?>
                </div>
            <?php endif; ?>

            <form method="POST" id="signupForm">
                <div class="form-group">
                    <label for="nom">Nom</label>
                    <input type="text" id="nom" name="nom" placeholder="Votre nom" required
                           value="<?php echo isset($_POST['nom']) ? htmlspecialchars($_POST['nom']) : ''; ?>">
                </div>

                <div class="form-group">
                    <label for="prenom">Prénom</label>
                    <input type="text" id="prenom" name="prenom" placeholder="Votre prénom" required
                           value="<?php echo isset($_POST['prenom']) ? htmlspecialchars($_POST['prenom']) : ''; ?>">
                </div>

                <div class="form-group">
                    <label for="email">Email</label>
                    <input type="email" id="email" name="email" placeholder="votre@email.com" required
                           value="<?php echo isset($_POST['email']) ? htmlspecialchars($_POST['email']) : ''; ?>">
                </div>

                <div class="form-group">
                    <label for="motdepasse">Mot de passe</label>
                    <input type="password" id="motdepasse" name="motdepasse" 
                           placeholder="Min 8 caractères, 1 chiffre, 1 spécial" required>
                </div>

                <div class="form-group">
                    <label for="motdepasse2">Confirmer mot de passe</label>
                    <input type="password" id="motdepasse2" name="motdepasse2" 
                           placeholder="Confirmez votre mot de passe" required>
                </div>

                <button type="submit">Créer mon compte</button>
            </form>

            <div class="form-footer">
                <p>Vous avez déjà un compte ? <a href="login.php">Se connecter</a></p>
            </div>
        </div>
    </div>

    <script>
        // Validation en temps réel
        const form = document.getElementById('signupForm');
        const fields = ['nom', 'prenom', 'email', 'motdepasse', 'motdepasse2'];

        fields.forEach(fieldName => {
            const input = document.getElementById(fieldName);
            input.addEventListener('input', function() {
                validateField(fieldName);
            });
        });

        function validateField(fieldName) {
            const input = document.getElementById(fieldName);
            let isValid = false;

            if (fieldName === 'nom' || fieldName === 'prenom') {
                isValid = input.value.trim().length >= 2;
            } else if (fieldName === 'email') {
                isValid = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(input.value);
            } else if (fieldName === 'motdepasse') {
                const pwd = input.value;
                isValid = pwd.length >= 8 && /[0-9]/.test(pwd) && /[!@#$%^&*]/.test(pwd);
            } else if (fieldName === 'motdepasse2') {
                const pwd1 = document.getElementById('motdepasse').value;
                isValid = input.value === pwd1 && input.value.length > 0;
            }

            if (input.value.length > 0) {
                if (isValid) {
                    input.classList.remove('invalid');
                    input.classList.add('valid');
                } else {
                    input.classList.remove('valid');
                    input.classList.add('invalid');
                }
            } else {
                input.classList.remove('valid', 'invalid');
            }
        }
    </script>
</body>
</html>